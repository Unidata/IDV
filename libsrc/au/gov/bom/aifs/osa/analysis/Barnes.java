...
/*
 * The Barnes library is part of the Australian Integrated Forecast System
 * (AIFS).
 *
 ...
 */
/**
 * Title: Barnes Objective Analysis (BOA) Scheme <p> Description: The BOA is
 * implemented as the static method "point2grid" and is for converting
 * irregularly spaced data defined by (longitude, latitude, value) to a regular,
 * but not necessarily evenly spaced, grid whose coordinates are also defined by
 ...
 *
 * <p> References: <p> Barnes, S.L., 1994a: Applications of the Barnes objective
 * analysis scheme Part I: Effects of undersampling, wave position, and station
 * randomness. J. Atmos. Oceanic Technol. 11, 1433-1448. <p> Barnes, S.L.,
 * 1994b: Applications of the Barnes objective analysis scheme Part II:
 * Improving derivative estimates. J. Atmos. Oceanic Technol. 11, 1449-1458. <p>
...
public class Barnes {
    ...
    /**
     * Description: point2grid is for converting irregularly spaced data defined
     * by (longitude, latitude, value) to a regular, but not neccessarily evenly
     * spaced, grid whose coordinates are also defined by longitude and latitude
     * inputs
     *
     * @param lon : the longitudes on the grid where interpolated values are
     * desired (in degrees)
     * @param lat : the latitudes on the grid where interpolated values are
     * desired (in degrees)
     * @param data3D : 3D array of data values where data3D[0][*] = longitudes
     * of data values data3D[1][*] = latitudes of data values data3D[2][*] =
     * data values
     *
     * @param scaleLength : the Gaussian scale length (in grid units) should be
     * approximately equal to the average data spacing a suggested default:
     * 10.0f (no scientific reason here)
     * @param gain factor by which scaleLength is reduced for the second pass.
     * Should be in the range 0.2 to 1.0. Data are fitted more closely with a
     * gain of 0.2 (at the expense of less overall accuracy over the entire
     * grid); larger values smooth more. Suggested default: 1.0f
     * @param iNumPasses number of passes of the BOA to do. 4 passes recommended
     * for analysing fields where derivative estimates are important (Ref:
     * Barnes 1994b) 3 passes recommended for all other fields (with gain set to
     * 1.0) (Ref: Barnes 1994c "Two pass Barnes Objective Analysis schemes now
     * in use probably should be replaced by appropriately tuned 3pass or 4pass
     * schemes") 2 passes only recommended for "quick look" type analyses
     *
     ...
     * uses a supplied first guess field (eg. a model background)
     */
    public synchronized static float[][] point2grid(float[] lon,
            ...
            for (int i = 0; i < numLon; i++) {
                for (int j = 0; j < numLat; j++) {
                    ...
                    for (int k = 0; k < numData; k++) {
                        ...
                        if ((!Float.isNaN(faDifferences[k]))
            ...
            if ((iPass < iNumPasses) || (reportRMSErrors)) {
                for (int k = 0; k < numData; k++) {
                    ...
                    float fInterpolatedData =
                            ...
                            faLatValues[k] + 1,
                            faaGrid);
                    //GHANSHAM:This check has been introduced to take 
                    //care of the case when there is no first guess and
                    //we have filled the grid values to NaN which dont 
                    //lie within the input radii
                    if (!Float.isNaN(fInterpolatedData)) { 
                        faDifferences[k] =
...
//      float gridMean = Barnes.mean(faValues);        
        for (int i = 0; i < numLon; i++) {
            for (int j = 0; j < numLat; j++) {
//GHANSHAM:commented this. Actually the grid was filled with the mean beforehand.
...
//GHANSHAM:As proposed by Don M, just NaNed the grid values for which 
//weighing function was negligible.
                    faaGrid[i][j] = Float.NaN; 
                }
            }
        }
     ...
     *
     * @param fLonMin : minimum longitude for grid
     * @param fLatMin : minimum latitude for grid
     * @param fLonMax : maximum longitude for grid
     * @param fLatMax : maximum latitude for grid
     * @param data3D : 3D array of data values
     * @return float[numLon][numLat] : regular gridded data
     */
     ...
     *
     * All the user needs to provide is the data to be analysed, and the domain
     * over which the analysis is to be carried out
     *
     * @param fLonMin : minimum longitude for grid (eg 140.0 = 140.0 E)
     * @param fLatMin : minimum latitude for grid (eg -40.0 = 40.0 S)
     * @param fLonMax : maximum longitude for grid (eg 150.0 = 150.0 E)
     * @param fLatMax : maximum latitude for grid (eg -30.0 = 30.0 S)
     * @param data3D : 3D array of data values (3, ndata) where ndata is the
     * number of data points, and can be any number larger than N. each row of
     * data should contain a longitude, a latitude, and a value to be
     * interpolated.
     * @param iNumPasses number of passes of the BOA to do.
     * @return float[numLon][numLat] : regular gridded data
     *
     */
    public static float[][] point2grid(float fLonMin, float fLatMin,
     ...
     * @param data3D : 3D array of irregularly spaced data values
     * @param daaGrid : regular gridded data
     * @return float[] daDifferences: an array of all the differences (between
     * the irregular data and the value interpolated from the analysed grid)
     */
     ...
     * @param fLonMin : minimum longitude for grid
     * @param fLonMax : maximum longitude for grid
     * @param gridX : the length of 1 grid space in X direction (degrees)
     ...
     * @param fLatMin : minimum latitude for grid
     * @param fLatMax : maximum latitude for grid
     * @param gridY : the length of 1 grid space in Y direction (degrees)
    ...
    public Barnes() {
        ...
        float[][] data3D = {{140.0f, 150.0f, 160.0f},
            {-40.0f, -50.0f, -60.0f},
            {1.0f, 2.0f, 3.0f}};
        // data3D[0][*] = {lons}
        // data3D[1][*] = {lats}
     ...
     * multiple values such as: gridX, gridY, scaleLengthGU, randomDataSpacing,
     * faGridX, faGridY gridX:the length of 1 grid space in the X direction
     * (degrees) (set to 0.3 * random data spacing) gridY: the length of 1 grid
     * space in the Y direction (degrees) (set to 0.3 * random data spacing)
     * scaleLengthGU: Recommended Gaussian length scale (in grid units). (set to
     * random data spacing) randomDataSpacing: Average spacing between station
     * if stations were randomly distributed (km) faGridX: the array of
     * recommended grid points along the x axis faGridY: array of recommended
     * grid points along the y axis actualStationSpacing (Not implemented): The
     * average distance between each station and its nearest neighbour NUR (Not
     * implemented): Non Uniformity Ratio Indicates how uniform data
     * distribution is 0 = perfectly regular data grid >= 1.1 indicates a pseudo
     * random distribution
     */
    public static class AnalysisParameters {
         ...
         *
         * @param scaleLengthGU scale length in Grid Units
         */
         ...
         *
         * @param faGridX the X Grid points
         */
         ...
         *
         * @param faGridX the Y Grid points
         */
         ...
         * @param gridX : the length of 1 grid space in X direction (degrees)
         * @param gridY : the length of 1 grid space in Y direction (degrees)
         * @param sl : Scale Length parameter
         * @param r : Random Data Spacing
         */
         ...
         * @param sl : Scale Length parameter
         * @param r : Random Data Spacing
         * @param faLonGrid : array of grid points in X direction
         * @param faLatGrid : array of grid points in Y direction
         */
        public AnalysisParameters(double gridX, double gridY,
     ...
     * recommendations on these parameters. <p> The literature (eg Barnes 1994
     * a,b,c) contains a lot of guidance on how to select these parameters, all
     * of which can be derived directly from the average station spacing. <p>
     * Strictly speaking the actual spacing between each station and its nearest
     * neighbours should be used (Barnes used the 6 nearest neighbours), but the
     * routine below uses the simpler approach of assuming a random spacing of
     * the stations, and calculates the random data spacing via a simple
     * formula. <p>
     *
     * @param fLonMin : Minimum Longitude of analysis area (eg 140.0 = 140.0 E)
     * @param fLatMin : Minimum Latitude of analysis area (eg -40.0 = 40.0 S)
     * @param fLonMax : Maximum Longitude of analysis area (eg 150.0 = 150.0 E)
     * @param fLatMax : Maximum Latitude of analysis area (eg -30.0 = 30.0 S)
     * @param faa3DData : An array (3,ndata) where ndata is the number of data
     * points. Each row of data should contain a longitude, a latitude, and a
     * value to be interpolated.
     * @return AnalysisParameters : see inner class for description of member
     * variables
    ...
    public static synchronized AnalysisParameters getRecommendedParameters(
         ...
         * this then lets us calculate the "Non-Uniformity Ratio" // ... double
         * actualStationSpacing = stationSpacing(faa3DData) NUR =
         * (randomDataSpacing-actualStationSpacing)/ actualStationSpacing if
         * (NUR lt 0.0) then NUR = 0.0
         */
    ...
    /**
     * Calculate the number of kms between 2 locations specified by longitude
     * and latitude: (fLon1, fLat1) and (fLon2, fLat2)
     *
     * @param fLon1 : longitude of first point
     * @param fLat1 : latitude of first point
     * @param fLon2 : longitude of second point
     * @param fLat2 : latitude of second point
     *
     * The return value is provided as a 2 element array, giving km in the x
     * direction and km in the y direction between the 2 points:
     * @return double[] where double[0] = km in x direction (dKmx) and double[1]
     * = km in y direction (dKmy)
     */
    public static double[] deg2km(float fLon1, float fLat1, float fLon2,
        ...
        double dKmx =
                ...
                * ((111415.1d * Math.cos(dAvgLat))
                - (94.54999d * Math.cos(3.0d * dAvgLat)) - (0.12d * Math.cos(5.0d * dAvgLat)));
        double dKmy =
                (double) ((fLat2 - fLat1))
                / 1000.0d
                * (111132.1d
                - (566.05d * Math.cos(2.0d * dAvgLat))
                + (1.2d * Math.cos(4.0d * dAvgLat)) - (0.003d * Math.cos(6.0d * dAvgLat)));
        double[] daKm = new double[2];
        daKm[0] = dKmx;
        daKm[1] = dKmy;
    ...
    /**
     * Using a model field as a first guess presents a problem if the model does
     * not completely cover the area of the analysis. Parts of the first guess
     * will contain NaN. This method allows NaN values to be filtered out and
     * replaced by another value
     *
     * @param array The array of values, possibly containing NaN
     * @param nonNaN The replacement value for NaN @returns the array of values,
     * with NaN replaced by nonNaN
     */
     ...
     * Name: scinex
     *
     * PURPOSE: This function returns the value scint of a scalar field at a
     * point gm,gn by interpolation or extrapolation of the field scala
     * (Lagrangian cubic formula).
     *
     * CALLING SEQUENCE: scint = scinex(gm,gn,scala)
     *
     * @param gm: The x-coordinate.
     * @param gn: The y-coordinate.
     * @param scala: A 2-d array (scalar field).
     * @return double: The interpolated data value (scint)
     *
     * MODIFICATION HISTORY: written by: Robert Dahni rrd@bom.gov.au 04/01/95
     * (IDL version) James Kelly J.Kelly@bom.gov.au Jan 2001 (Java version)
     */
