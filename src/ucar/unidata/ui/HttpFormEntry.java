...
public class HttpFormEntry {
	...
	HTTPFormBuilder builder = new HTTPFormBuilder();
		...
		build.add(formEntry.getName(),formEntry.getBytes(),formEntry.getFileName());
	    } else
		    build.add(formEntry.getName(),formEntry.getValue());
	}	
        postMethod.setRequestEntity(builder.build());
        return postMethod;
    }
}
