...
public class IdvComponentGroup  extends ComponentGroup {
    ...
    protected List getPopupMenuItems(List items) {
        ...
        newItems.add(GuiUtils.makeMenuItem("Map View", this, "makeNew",IdvUIManager.COMP_MAPVIEW));
        newItems.add(GuiUtils.makeMenuItem("Globe View", this, "makeNew",IdvUIManager.COMP_GLOBEVIEW));
        newItems.add(GuiUtils.makeMenuItem("Transect View", this, "makeNew",IdvUIManager.COMP_TRANSECTVIEW));
        newItems.add(GuiUtils.makeMenuItem("Field Selector", this, "makeNew",IdvUIManager.COMP_DATASELECTOR));
        newItems.add(GuiUtils.makeMenuItem("Tab Group", this, "makeNew",IdvUIManager.COMP_COMPONENTGROUP));
        ...
        for(int vmIdx=0;vmIdx<vms.size();vmIdx++) {
            ...
            if(displayControls.size()>0) {
                viewItems.add(GuiUtils.makeMenuItem("Import All",this,"importAllDisplayControls",displayControls));            
            ...
            for (int i = 0; i < displayControls.size(); i++) {
                ...
                viewItems.add(GuiUtils.makeMenuItem(dc.getLabel(),this,"importDisplayControl",dc));            
            }
            if(viewItems.size()>0) {
    ...
    public void makeNew(String what) {
        ...
        if(what.equals(IdvUIManager.COMP_MAPVIEW)) {
            ViewManager vm = new MapViewManager(idv,new ViewDescriptor(),"showControlLegend=false");
            ...
            MapViewManager vm = new MapViewManager(idv,new ViewDescriptor(),"showControlLegend=false");
            vm.setUseGlobeDisplay(true);
            idv.getVMManager().addViewManager(vm);
            addComponent(new IdvComponentHolder(idv,vm));
        } else   if(what.equals(IdvUIManager.COMP_TRANSECTVIEW)) {
            ViewManager vm = new TransectViewManager(idv,new ViewDescriptor(),"showControlLegend=false");
            idv.getVMManager().addViewManager(vm);
            addComponent(new IdvComponentHolder(idv,vm));
        } else   if(what.equals(IdvUIManager.COMP_DATASELECTOR)) {
            addComponent(new IdvComponentHolder(idv,idv.getIdvUIManager().createDataSelector(false,false)));
