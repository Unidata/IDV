/*
 * $Id: Resource.java,v 1.14 2006/05/05 19:19:37 jeffmc Exp $
 *
...
public class Resource {
    ...
    public static ImageIcon getIcon(String fullIconName, boolean errMsg) {
        ...
        if (debugIcon) {
            ...
            System.out.println("  Resource.getIcon on " + fullIconName
                               + " = " + iconR);
        }
        ...
        if ((icon == null) && errMsg) {
            System.out.println("  ERROR: Resource.getIcon failed on "
                               + fullIconName);
        } else if (debugIcon) {
    ...
    public static Image getImage(String fullImageName) {
        ...
        if (image == null) {
            System.out.println(
                "  ERROR: Resource.getImageResource failed on "
                + fullImageName);
        }
        return image;
    }
    ...
    public static ImageIcon getImageIcon(String fullImageName) {
        return new ImageIcon(getImage(fullImageName));
    }
    ...
    public static Cursor makeCursor(String name) {
        ...
        try {
            ...
            if (debug) {
                ...
                System.out.println(
                    " bestCursorSize = "
                    + tk.getBestCursorSize(
                        image.getWidth(obs), image.getHeight(obs)));
                System.out.println(" getMaximumCursorColors = "
                                   + tk.getMaximumCursorColors());
            }
            cursor = tk.createCustomCursor(image, new Point(17, 17), name);
        } catch (IndexOutOfBoundsException e) {
            System.out.println("NavigatedPanel createCustomCursor failed "
                               + e);
            return null;
        }
        return cursor;
    ...
    public static InputStream getFileResource(String resourcePath,
        ...
        if (is != null) {
            if (debug) {
                System.out.println("Resource.getResourceAsStream ok on "
                                   + fullName);
            }
            return is;
        } else if (debug) {
            System.out.println("Resource.getResourceAsStream failed on <"
                               + fullName + ">");
        }
        ...
        try {
            ...
            if (debug) {
                System.out.println("Resource.FileInputStream ok on "
                                   + fullName);
            }
