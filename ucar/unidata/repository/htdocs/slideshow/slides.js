...
var s5NotesWindow;
//jeffmc:
var haveTriedToOpenNotesWindow  = false;
...
function notOperaFix() {
	...
	if (isIE && document.styleSheets && document.styleSheets[0]) {
		document.styleSheets[0].addRule('img', 'behavior: url(${urlroot}/slideshow/iepngfix.htc)');
		document.styleSheets[0].addRule('div', 'behavior: url(${urlroot}/slideshow/iepngfix.htc)');
		document.styleSheets[0].addRule('.slide', 'behavior: url(${urlroot}/slideshow/iepngfix.htc)');
	}
}
...
function createNotesWindow() { // creates a window for our notes
	if (!s5NotesWindow || s5NotesWindow.closed) { // Create the window if it doesn't exist
		...
		if(!haveTriedToOpenNotesWindow) {
		    var notesUrl = '${urlroot}/slideshow/s5-notes.html';
		    s5NotesWindow = window.open(notesUrl, 's5NotesWindow', 'top=0,left=0');
		}
		haveTriedToOpenNotesWindow = true;
