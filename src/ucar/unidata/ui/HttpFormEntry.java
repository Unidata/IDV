...
public class HttpFormEntry
{
    ...
    /**
     * file part source; might be filebody or bytearray
     */
    private ContentBody filebody = null;
                         ...
                         final byte[] bytes)
    {
        this.name = name;
        type = TYPE_FILE;
        this.filebody = new ByteArrayBody(bytes, fileName);
    }
     ...
     */
    private FileBody getFileBody()
    {
        if (filebody == null) {
            final String file = getValue();
            filebody = new FileBody(new File(file));
        }
        return (FileBody) filebody;
    }
     ...
     *
     * @param entries The entries (actually List<HttpFormEntry>)
    ...
    public static String[] doPost(List entries, String urlPath)
    {
        try {
            ...
            int numTries = 0;
            while (numTries++ < 5) {
                postMethod = getPostMethod(entries, urlPath);
                code = postMethod.execute();
            ...
            String result =
                    IOUtil.readContents(postMethod.getResponseAsStream());
            if (code >= 300) {
    ...
    private static HTTPMethod getPostMethod(List entries, String urlPath)
            throws HTTPException
    {
        ...
        if (anyFiles) {
            for (int i = 0; i < goodEntries.size(); i++) {
                ...
                if (formEntry.type == TYPE_FILE) {
                    mpeb.addPart(formEntry.getName(), formEntry.getFileBody());
                } else {
                    ...
                    String value = formEntry.getValue().replace('\0', ' ');
                    try {
                        mpeb.addPart(formEntry.getName(), new StringBody(value));
            ...
            for (int i = 0; i < goodEntries.size(); i++) {
                HttpFormEntry formEntry =
                        (HttpFormEntry) goodEntries.get(i);
                try {
                    StringBody value = new StringBody(formEntry.getValue().replace('\0', ' '));
                    mpeb.addPart(formEntry.getName(), value);
                } catch (UnsupportedEncodingException uee) {
                    throw new HTTPException(uee);
                }
            }
        }
        postMethod.setRequestContent(mpeb);
        return postMethod;
    }
