...
public class JythonTypeHandler extends GenericTypeHandler {
	...
	if(makeForm)  {
	    ...
	    Result result = new Result(formInfo.title!=null?formInfo.title:entry.getName(), new StringBuffer(formInfo.resultHtml));
	    return result;
	...
	public void addFormFileUpload(String id, String label) {
	    ...
	    sb.append(HtmlUtil.formEntry(typeHandler.msgLabel(label),
					 HtmlUtil.fileInput(id, HtmlUtil.attr(HtmlUtil.ATTR_WIDTH,"80"))));
	}
	...
	public void addFormText(String id, String label, String dflt, int columns, int rows) {
	    ...
	    if(rows==1) {
		sb.append(HtmlUtil.formEntry(typeHandler.msgLabel(label),
					     HtmlUtil.input(id, dflt, HtmlUtil.attr(HtmlUtil.ATTR_WIDTH,""+columns))));
	    } else {
		sb.append(HtmlUtil.formEntry(typeHandler.msgLabel(label),
	...
	public void addFormNumber(String id, String label, double dflt) {
	    ...
	    cnt++;
	    sb.append(HtmlUtil.formEntry(typeHandler.msgLabel(label),
					 HtmlUtil.input(id, ""+dflt, HtmlUtil.attr(HtmlUtil.ATTR_WIDTH,""+5))));
	}
