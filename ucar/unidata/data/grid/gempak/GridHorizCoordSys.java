...
public class GridHorizCoordSys {
  ...
  GridHorizCoordSys(GridDefRecord gdsIndex, GridTableLookup lookup, Group g) {
    ...
    if (isLatLon && (lookup.getProjectionType(gdsIndex) == GridTableLookup.GaussianLatLon)) {
      isGaussian = true;
      double np = 90.0;
  ...
  void addDimensionsToNetcdfFile(NetcdfFile ncfile) {
    ...
    if (isLatLon) {
      ncfile.addDimension(g, new Dimension("lat", (int) getParamValue(gdsIndex.NY), true));
      ncfile.addDimension(g, new Dimension("lon", (int) getParamValue(gdsIndex.NX), true));
    } else {
      ncfile.addDimension(g, new Dimension("y", (int) getParamValue(gdsIndex.NY), true));
      ncfile.addDimension(g, new Dimension("x", (int) getParamValue(gdsIndex.NX), true));
    }
  }
  ...
  void addToNetcdfFile(NetcdfFile ncfile) {
    ...
    if (isLatLon) {
      double dy = (getParamValue("La2") < getParamValue(gdsIndex.LA1)) ? -getParamValue(gdsIndex.DY) : getParamValue(gdsIndex.DY);
      if (isGaussian)
        addGaussianLatAxis(ncfile, "lat", "degrees_north", "latitude coordinate", "latitude", AxisType.Lat);
      else
        addCoordAxis(ncfile, "lat", (int) getParamValue(gdsIndex.NY), getParamValue(gdsIndex.LA1), dy, "degrees_north", "latitude coordinate", "latitude", AxisType.Lat);

      addCoordAxis(ncfile, "lon", (int) getParamValue(gdsIndex.NX), getParamValue(gdsIndex.LO1), getParamValue(gdsIndex.DX), "degrees_east", "longitude coordinate", "longitude", AxisType.Lon);
      addCoordSystemVariable(ncfile, "latLonCoordSys", "time lat lon");

    } else {
      makeProjection(ncfile);
      double[] yData = addCoordAxis(ncfile, "y", (int) getParamValue(gdsIndex.NY), starty, getDyInKm(), "km",
  ...
  private double[] addGaussianLatAxis(NetcdfFile ncfile, String name, String units,
    ...
    assert Math.abs(bestEndIndex - bestStartIndex + 1) == getParamValue(gdsIndex.NY);
  ...
  private void makeProjection(NetcdfFile ncfile) {
    switch (lookup.getProjectionType(gdsIndex)) {
      ...
      case GridTableLookup.Orthographic:
        ...
        break;
      default:
        throw new UnsupportedOperationException("unknown projection = " + getParamValue(gdsIndex.GRID_TYPE));
    }
    ...
    if (getParamValue(gdsIndex.GRID_SHAPE_CODE) == 1) {
      v.addAttribute(new Attribute("spherical_earth_radius_meters", new Double(getParamValue(gdsIndex.RADIUS_SPHERICAL_EARTH))));
    }
  ...
  private void makeLC() {
    ...
    proj = new LambertConformal(getParamValue(gdsIndex.LATIN1), getParamValue(gdsIndex.LOV), getParamValue(gdsIndex.LATIN1), getParamValue(gdsIndex.LATIN2));
    LatLonPointImpl startLL = new LatLonPointImpl(getParamValue(gdsIndex.LA1), getParamValue(gdsIndex.LO1));
    ...
    if (getParamValue(gdsIndex.LATIN1) == getParamValue(gdsIndex.LATIN2))
      attributes.add(new Attribute("standard_parallel", new Double(getParamValue(gdsIndex.LATIN1))));
    else {
      double[] data = new double[]{getParamValue(gdsIndex.LATIN1), getParamValue(gdsIndex.LATIN2)};
      attributes.add(new Attribute("standard_parallel",
              Array.factory(DataType.DOUBLE.getClassType(), new int[]{2}, data)));
    }
    attributes.add(new Attribute("longitude_of_central_meridian", new Double(getParamValue(gdsIndex.LOV))));
    attributes.add(new Attribute("latitude_of_projection_origin", new Double(getParamValue(gdsIndex.LATIN1))));
  ...
  private void makePS() {
    ...
    proj = new Stereographic(latOrigin, getParamValue(gdsIndex.LOV), scale);

    // we have to project in order to find the origin
    ProjectionPointImpl start = (ProjectionPointImpl) proj.latLonToProj(new LatLonPointImpl(getParamValue(gdsIndex.LA1), getParamValue(gdsIndex.LO1)));
    startx = start.getX();
    starty = start.getY();
    ...
    if (debug) {
      ...
      System.out.println("   should be lat=" + getParamValue(gdsIndex.LA1) + " lon=" + getParamValue(gdsIndex.LO1));
    }

    attributes.add(new Attribute("grid_mapping_name", "polar_stereographic"));
    attributes.add(new Attribute("longitude_of_projection_origin", new Double(getParamValue(gdsIndex.LOV))));
