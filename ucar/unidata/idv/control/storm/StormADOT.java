...
public class StormADOT {
   ...
   int aodtv72_drive( int curdate, int curtime, int cursat,
       ...
       /*
        ...
        iaodt=aodtv72_setdomain ( g_domain );



        /*
         *  Retrieve temperatures from image. This need to be done in IDV
         */
         ...
         */
         ...
         odtcurrent_v72IR = aodtv72_seteyecloudtemp (StormADOTInfo.keyerM_v72, areadata_v72 );

          /*
         *   Determine scene type
         */
          StormADOTSceneType sceneType = new StormADOTSceneType();

          float [] oscen = sceneType.aodtv72_calcscene( odtcurrent_v72IR, rmwsizeman_v72, areadata_v72, osstr_v72, osearch_v72);
     ...
     *--------------------------------------*/
     ...
     StormADOTInfo.IRData aodtv72_seteyecloudtemp(int keyerM_v72, StormADOTInfo.DataGrid areadata)
    ...
    */
    {
      ...
      return sceneType.aodtv72_gettemps(keyerM_v72, areadata);
    ...
    */
    {
      ...
      if((odtcurrent_v72IR.longitude<-180.)||(odtcurrent_v72IR.longitude>180.)) iret=-21;
      if((odtcurrent_v72IR.latitude<-90.)||(odtcurrent_v72IR.latitude>90.)) iret=-21;
    ...
    */
    ...
    public int aodtv72_calcintensity( int idomain_v72)
    ...
    */
    {
        ...
        /* calculate current Raw T# value */
            odtcurrent_v72IR.Traw=aodtv72_Tnoraw(odtcurrent_v72IR, idomain_v72);
            ...
            if(true) {
              ...
              odtcurrent_v72IR.CIadjp = aodtv72_latbias(odtcurrent_v72IR.CI,odtcurrent_v72IR.latitude,
                      odtcurrent_v72IR.longitude, odtcurrent_v72IR);
    ...
    float aodtv72_latbias(float initval,float latitude,float longitude, StormADOTInfo.IRData odtcurrent_v72IR)
    ...
    */
    ...
    int aodtv72_initcurrent(boolean redo , StormADOTInfo.IRData odtcurrent_v72IR)
    ...
    float aodtv72_Tnoraw(StormADOTInfo.IRData odtcurrent_v72IR, int idomain_v72)
        /* Compute initial Raw T-Number value using original Dvorak rules
            Inputs  : global structure odtcurrent_v72 containing current analysis
        ...
        */
    {
        double eno[][]=    { {1.00,2.00,3.25,4.00,4.75,5.50,5.90,6.50,7.00,7.50,8.00},   /* original plus adjusted > CDG+ */
                         {1.50,2.25,3.30,3.85,4.50,5.00,5.40,5.75,6.25,6.50,7.00} };   /* adjusted based     */
        double cdo[][]  =  { {2.00,2.40,3.25,3.50,3.75,4.00,4.10,4.20,4.30,4.40,4.70},
        ...
        double diffchk[][] = { {0.0,0.5,1.2,1.7,2.2,2.7,0.0,0.0,0.1,0.5 },   /* shear scene types... original Rule 8 rules */
        ...
        cloudtemp=odtcurrent_v72IR.cloudt;
        eyetemp=odtcurrent_v72IR.eyet;
        cloudcat = 0;
        eyecat = 0;
        lastci=4.0f;
        xpart = 0.0f;
        ...
        /* compute cloud category */
            if((cloudtemp<=StormADOTInfo.ebd_v72[ixx])&&(cloudtemp>StormADOTInfo.ebd_v72[ixx+1])) {
              cloudcat=ixx;
              xpart=(float)(cloudtemp-StormADOTInfo.ebd_v72[cloudcat])/
                     (float)(StormADOTInfo.ebd_v72[cloudcat+1]-StormADOTInfo.ebd_v72[cloudcat]);
            }
            /* compute eye category for eye adjustment */
            if((eyetemp<=StormADOTInfo.ebd_v72[ixx])&&(eyetemp>StormADOTInfo.ebd_v72[ixx+1])) {
              eyecat=ixx;
            }
        /* eyetemp=Math.min(0.0,eyetemp); */
        }
        if(odtcurrent_v72IR.eyescene==1) {
        ...
        /* if scenetype is EYE */
        rp=odtcurrent_v72IR.ringcbval;
        rb=odtcurrent_v72IR.ringcb;
        fftcloud=odtcurrent_v72IR.cloudfft;

        if(odtcurrent_v72IR.cloudscene==3) {
            /* CURVED BAND */
            rp=Math.min(30,rp+1);   /* added 1 for testing */
            xrp=rp/5;
            incval=0.1f;
            if(xrp==1) incval=0.2f;
            ddvor=(float)curbnd[xrp];
            xaddtno=incval*(float)(rp-(xrp*5));
            /* printf("rp=%d  xrp=%d  rb=%d  ddvor=%f  xaddtno=%f\n",rp,xrp,rb,ddvor,xaddtno); */
            ddvor=ddvor+xaddtno;
            if(rb==5) ddvor=Math.min(4.0f,ddvor+0.5f);
            if(rb==6) ddvor=Math.min(4.5f,ddvor+1.0f);
            diffchkcat=2;   /* added for test - non-eye/shear cases */
        } else if(odtcurrent_v72IR.cloudscene==4) {
            /* POSSIBLE SHEAR -- new definition from NHC */
            ixx=0;
            ddvor=1.0f;
            sdist=odtcurrent_v72IR.eyecdosize;  /* shear distance */
            while(ixx<5) {
            ...
            /* EYE or NO EYE */
            if(odtcurrent_v72IR.eyescene<=2) {
                /* EYE */
                ...
                symadj=(float)symadjfacEYE[idomain_v72]*(odtcurrent_v72IR.cloudsymave);
                ...
                /* printf("ddvor=%f\n",ddvor); */
                if(odtcurrent_v72IR.eyescene==2)  ddvor=Math.min(ddvor-0.5f,6.5f); /* LARGE EYE adjustment */
                ...
                dgraysizeadj=(float)dgraysizefacCLD[idomain_v72]*odtcurrent_v72IR.eyecdosize;
                /* printf("CDO : dgraysize=%f  symadj=%f\n",odtcurrent_v72->IR.eyecdosize,dgraysizeadj); */
                /* symadj=-0.03*(odtcurrent_v72->IR.cloudsymave); */
                symadj=(float)symadjfacCLD[idomain_v72]*(odtcurrent_v72IR.cloudsymave);
                ...
                ciadj=0.0f;
                if(odtcurrent_v72IR.cloudscene==0) { /* CDO */
                if(lastci>=4.5) ciadj=Math.max(0.0f,Math.min(1.0f,lastci-4.5f));
                if(lastci<=3.0) ciadj=Math.min(0.0f,Math.max(-1.0f,lastci-3.0f));
                    /* printf("CDO : lastci=%f   xaddtno=%f\n",lastci,ciadj); */
                    ddvor=ddvor+ciadj;
                }
                if(odtcurrent_v72IR.cloudscene==1) { /* EMBEDDED CENTER */
                    ciadj=Math.max(0.0f,Math.min(1.5f,lastci-4.0f));
                    /* printf("EMBC : lastci=%f   xaddtno=%f\n",lastci,ciadj); */
                    ddvor=ddvor+ciadj;   /* changed from 0.5 */
                }
                if(odtcurrent_v72IR.cloudscene==2) { /* IRREGULAR CDO (PT=3.5) */
