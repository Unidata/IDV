...
public class GridIndexToNC  {
  ...
  static public String makeLevelName(GridRecord gr, GridTableLookup lookup) {
  ...
  static public String makeVariableName(GridRecord gr, GridTableLookup lookup) {
  ...
  void open(GridIndex index, GridTableLookup lookup, int version, NetcdfFile ncfile, FmrcCoordSys fmrcCoordSys, CancelTask cancelTask) throws IOException {
    ...
    if (debug) System.out.println(" number of products = "+records.size());
    for( int i = 0; i < records.size(); i++ ) {
      GridRecord gribRecord = (GridRecord) records.get( i );
      if (firstRecord == null) firstRecord = gribRecord;

      GridHorizCoordSys hcs = (GridHorizCoordSys) hcsHash.get(gribRecord.getGridDefRecordId());
