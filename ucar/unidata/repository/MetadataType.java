...
public class MetadataType implements Constants {
    ...
    private static void parse(Element root, MetadataManager manager,List<MetadataType> types) 
        throws Exception {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            Class c = Misc.findClass(XmlUtil.getAttributeFromTree(node,
                                                                  ATTR_CLASS,"ucar.unidata.repository.MetadataHandler"));
            ...
            metadataType.catalogTemplate = XmlUtil.getGrandChildText(node, TAG_CATALOGTEMPLATE);


            handler.addMetadataType(metadataType);
            types.add(metadataType);
            metadataType.setAdminOnly(XmlUtil.getAttributeFromTree(node,ATTR_ADMINONLY,false));
            metadataType.setForUser(XmlUtil.getAttributeFromTree(node,ATTR_FORUSER,true));
            metadataType.setSearchable(XmlUtil.getAttributeFromTree(node,ATTR_SEARCHABLE,false));
            metadataType.setBrowsable(XmlUtil.getAttributeFromTree(node,ATTR_BROWSABLE,false));
            metadataType.setDisplayCategory(XmlUtil.getAttributeFromTree(node,ATTR_DISPLAYCATEGORY,"Metadata"));
            metadataType.setCategory(XmlUtil.getAttributeFromTree(node,ATTR_CATEGORY,handler.getHandlerGroupName()));
            ...
            for (int j = 0; j < elements.size(); j++) {
                ...
                MetadataElement element = new MetadataElement(metadataType,j+1,elementType,  XmlUtil.getAttribute(elementNode,ATTR_LABEL),
                                                              XmlUtil.getAttribute(elementNode,ATTR_ROWS,1),
                                                              XmlUtil.getAttribute(elementNode,ATTR_COLUMNS,60),null);
                element.setThumbnail(XmlUtil.getAttribute(elementNode, ATTR_THUMBNAIL, false));
                ...
                if(elementType.equals(MetadataElement.TYPE_ENUMERATION)) {
                    ...
                    if(values.startsWith("file:")) {
                        ...
                        tmpValues = (List<String>)StringUtil.split(values,",",true,true);
                    }
    ...
    public void newEntry(Metadata metadata, Entry entry) throws Exception {
        for (MetadataElement element : elements) {
            if(element.getType().equals(element.TYPE_FILE)) {
                ...
                if(fileArg==null || fileArg.length()==0) continue;
                if ( !entry.getIsLocalFile()) {
                    fileArg =
                        handler.getRepository().getStorageManager().copyToEntryDir(entry,
    ...
    public boolean processMetadataXml(Entry entry, Element node, Metadata metadata,
                                   Hashtable fileMap, boolean internal)
            throws Exception {
        for (MetadataElement element : elements) {
            if(!element.getType().equals(element.TYPE_FILE)) {continue;}
            String fileArg  = XmlUtil.getAttribute(node, ATTR_ATTR+element.getIndex(), "");
   ...
   /**
     ...
     * @throws Exception _more_
     */
    public void handleForm(Request request, Entry entry, Metadata metadata, String suffix,
        ...
        for (MetadataElement element : elements) {
            if(!element.getType().equals(element.TYPE_FILE)) {continue;}
            if(oldMetadata!=null) {
                metadata.setAttr(element.getIndex(), oldMetadata.getAttr(element.getIndex()));
            }

            String url     = request.getString(ARG_ATTR+element.getIndex() + suffix+".url", "");
            String theFile = null;
            if (url.length() > 0) {
                String tail    = IOUtil.getFileTail(url);
                File   tmpFile = handler.getStorageManager().getTmpFile(request,
                                     tail);
                ...
                String fileArg = request.getUploadedFile(ARG_ATTR+element.getIndex() + suffix);
                if (fileArg == null) {
                    continue;
                }
                theFile = fileArg;
            ...
            theFile =
                handler.getRepository().getStorageManager().moveToEntryDir(entry,
    ...
    public void addMetadataToCatalog(Request request, Entry entry,
        ...
        for (MetadataElement element : elements) {
            ...
            String  path = handler.getRepository().getMetadataManager().URL_METADATA_VIEW.getFullUrl("/" + tail);
            String url = HtmlUtil.url(path, ARG_ELEMENT,element.getIndex()+"", ARG_ENTRYID,
            ...
            XmlUtil.create(
                           ...
                           new String[] { ThreddsMetadataHandler.ATTR_NAME,
                                          (element.getThumbnail()? "thumbnail":"attachment"), ThreddsMetadataHandler.ATTR_VALUE, url});
        ...
        for(int attr=1;attr<=4;attr++) {
            template = template.replace("${attr" + attr+"}",metadata.getAttr(attr));
            template = template.replace("${attr" + attr+".cdata}","[CDATA[" +metadata.getAttr(attr)+"]]");
        }
        template = "<tmp>" + template +"</tmp>";
        Element root = XmlUtil.getRoot(new ByteArrayInputStream(template.getBytes()));
        NodeList children = XmlUtil.getElements(root);
        for (int i = 0; i < children.getLength(); i++) {
            Element node = (Element) children.item(i);
    ...
    private File getFile(Entry entry, Metadata metadata, MetadataElement element) {
        File f;
        if ( !entry.getIsLocalFile()) {
            f = new File(
                IOUtil.joinDir(
                    handler.getRepository().getStorageManager().getEntryDir(
                        metadata.getEntryId(), false), metadata.getAttr(element.getIndex())));
        } else {
    ...
    public void decorateEntry(Request request, Entry entry, StringBuffer sb,
        ...
        for (MetadataElement element : elements) {
            ...
            if (element.getThumbnail()) {
                String html = getFileHtml(request, entry, metadata, element, forLink);
                if (html != null) {
            ...
            if ( !forLink) {
                String html = getFileHtml(request, entry, metadata, element, false);
                if (html != null) {
                    sb.append(HtmlUtil.space(1));
                    sb.append(html);
                    sb.append(HtmlUtil.space(1));
    ...
    public Result processView(Request request, Entry entry, Metadata metadata)
        ...
        if(!element.getType().equals(element.TYPE_FILE)) return new Result("", "Cannot process view");
        File f = getFile(entry, metadata,element);
        if (f == null) {
    ...
    public String getFileHtml(Request request, Entry entry,
                              Metadata metadata, MetadataElement element, boolean forLink) {
        File   f    = getFile(entry, metadata,element);
        if (f == null) {
            return null;
        }
        ...
        String extra = (forLink
                        ...
                        : "");
        String tail = handler.getStorageManager().getFileTail(f.toString());
        String  path = handler.getRepository().getMetadataManager().URL_METADATA_VIEW+"/" + tail;


        if (ImageUtils.isImage(f.toString())) {
            String img =
                HtmlUtil
                    .img(HtmlUtil.url(path, ARG_ELEMENT,element.getIndex()+"", ARG_ENTRYID,
                                metadata.getEntryId(), ARG_METADATA_ID,
            ...
            if (forLink) {
                ...
                String imgUrl =
                    HtmlUtil
                        .url(path, ARG_ELEMENT,element.getIndex()+"",ARG_ENTRYID,
                                        metadata.getEntryId(),
                ...
                img = handler.getRepository().makePopupLink(img, bigimg, true, false);
            }
            return img;
        } else if (f.exists()) {
            String name =
                handler.getRepository().getStorageManager().getFileTail(f.getName());
    ...
    public String[] getForm(MetadataHandler handler, Request request,
        ...
        for (MetadataElement element : elements) {
            ...
            String widget     = element.getForm(request,entry,metadata, args[cnt], values[cnt],forEdit);
