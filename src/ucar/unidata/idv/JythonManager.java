...
public class JythonManager extends IdvManager implements ActionListener {
    ...
    /** jython extension */
    private String jyExt;

    /** local jython dir */
    private String jythonDir;

    /** The jython editor */
    private LibHolder mainHolder;

    /** python extension */
    private String pyExt;
    ...
    /** the jtree on the left that lists the different jython files */
    ...
    public JythonManager(IntegratedDataViewer idv) {
        super(idv);
        jythonDir = IOUtil.joinDir(getStore().getUserDirectory().toString(), "jython");
        pythonDir = IOUtil.joinDir(getStore().getUserDirectory().toString(), "python");

        if (!new File(jythonDir).exists()) {
            try {
                IOUtil.makeDir(jythonDir);

                // Move the old default.jy to the subdir
                File oldFile = new File(IOUtil.joinDir(getStore().getUserDirectory(), "default.jy"));
                File newFile = new File(IOUtil.joinDir(jythonDir, "default.jy"));

                if (oldFile.exists()) {
                    IOUtil.moveFile(oldFile, new File(jythonDir));
                } else {
        ...
        if (new File(pythonDir).exists()) {
            try {
                ...
                listing = IOUtil.getListing(pythonDir, null);

                for (String pyFileFull : listing) {
                    String pyFile  = new File(pyFileFull).getName();
                    File   oldFile = new File(IOUtil.joinDir(pythonDir, pyFile));
                    File   newFile;

                    if (IOUtil.getFileExtension(oldFile.toString()).equals(pyExt)) {
                        newFile = new File(IOUtil.joinDir(jythonDir, pyFile.replace(pyExt, jyExt)));
                    } else {
                        newFile = new File(jythonDir);
                    }

                    IOUtil.moveFile(oldFile, newFile);
                }

                IOUtil.deleteDirectory(new File(pythonDir));
            } catch (Exception exc) {
                logException("Moving old python lib to new jython lib", exc);
            }
        }
    ...
    private void writeJythonLib() {
        try {
            String jythonLibDir = IOUtil.joinDir(getStore().getJythonCacheDir(), "Lib");

            // double version     = 2.5;
            // String versionFile = IOUtil.joinDir(jythonLibDir, "version.txt");
            if (new File(jythonLibDir).exists()) {
            ...
            IOUtil.makeDir(jythonLibDir);

            InputStream    is  = IOUtil.getInputStream("/jythonlib.jar", getClass());
            ...
            while ((ze = zin.getNextEntry()) != null) {
                ...
                String dest = IOUtil.joinDir(jythonLibDir, entryName);
    ...
    /**
     * Initialize the Jython package in a thread.
     * We define the jython cache to be in the users
     * .unidata/idv directory. Jython puts the results of its
    ...
    /**
     * Initialize the jython interpreter. This gets called from initPython inside of a thread.
     */
    private void initPythonInner() {
        ...
        if (cacheDir != null) {
            jythonProps.put("jython.home", cacheDir);
        }

        PythonInterpreter.initialize(System.getProperties(), jythonProps, getArgsManager().commandLineArgs);
        doMakeContents();
    ...
    private void makeFormulasFromLib() {
        ...
        for (int i = 0; i < procedures.size(); i++) {
            ...
            if ((formulaId != null) || (desc != null)) {
                ...
                DerivedDataDescriptor ddd = new DerivedDataDescriptor(getIdv(), formulaId, ((desc != null)
                        ? desc
                        : func.__name__), makeCallString(
                                                                                                func,
                                                                                                attrProps), categories);
    ...
    protected JComponent doMakeContents() {
        ...
        try {
            ResourceCollection resources = getResourceManager().getResources(IdvResourceManager.RSC_JYTHON);
            ...
            for (int i = 0; i < resources.size(); i++) {
                ...
                if (file.exists() && file.isDirectory()) {
                    File[] libFiles = file.listFiles((java.io.FileFilter) new PatternFileFilter(".*\\.jy$"));
            ...
            String tmpPath = getIdv().getStore().getTmpFile("tmp.jy");
            ...
            contents = GuiUtils.topCenterBottom(menuBar, treePanel, textSearcher);
            setMenuBar(menuBar);
    ...
    public boolean saveOnExit() {
        ...
        if (toSave.size() > 0) {
            int response = GuiUtils.showYesNoCancelDialog(null,
                               "Do you want to save the modified Jython before exiting?", "Save Jython");
    ...
    private LibHolder makeLibHolder(boolean editable, String label, String path, String text) throws VisADException {
        ...
        final MyJythonEditor jythonEditor = new MyJythonEditor() {
        ...
        jythonEditor.getTextComponent().addMouseListener(new MouseAdapter() {
            public void mouseReleased(MouseEvent e) {
                ...
                if ((token != null) && (token.trim().length() > 0)) {
                    ...
                    for (int i = 0; i < funcs.size(); i++) {
                        ...
                        if (func.__name__.equals(token)) {
                            items.add(GuiUtils.makeMenuItem("Make formula for " + token, JythonManager.this,
                                                            "makeFormula", func));
                ...
                if (holderArray[0].isEditable()) {
                    ...
                    items.add(GuiUtils.makeMenu("Insert Idv Action",
                                                getIdv().getIdvUIManager().makeActionMenu(jythonEditor, "insertText",
                                                    true)));
                }
    ...
    public void makeFormula(PyFunction func) {
        ...
        for (int dddIdx = 0; dddIdx < descriptors.size(); dddIdx++) {
            DerivedDataDescriptor tmp = (DerivedDataDescriptor) descriptors.get(dddIdx);
        ...
        if (ddd != null) {
            ...
            if (!GuiUtils.askOkCancel("Formula Exists",
        ...
        if (ddd == null) {
            ddd = new DerivedDataDescriptor(getIdv(), name, "", makeCallString(func, null), new ArrayList());
        }
    ...
    public void makeNewLibrary() {
        ...
        try {
            while (true) {
                ...
                if (!fullName.endsWith(".jy")) {
                    fullName = fullName + ".jy";
                }

                fullName = IOUtil.joinDir(jythonDir, fullName);
            ...
            logException("An error occurred creating the jython library", exc);
        }
    }
    ...
    public void removeLibrary(LibHolder holder) {
        if (!GuiUtils.askYesNo("Remove Jython Library",
                               "Are you sure you want to remove the library: " + IOUtil.getFileTail(holder.filePath))) {
            return;
        }
    ...
    public void makeFileMenu(JMenu fileMenu) {
        ...
        if ((holder != null) && holder.isEditable() && (holder.editProcess == null)) {
            ...
            if (getStateManager().getPreferenceOrProperty(PROP_JYTHON_EDITOR, "").trim().length() > 0) {
                fileMenu.add(editFileMenuItem = GuiUtils.makeMenuItem("Edit in External Editor", this,
                        "editInExternalEditor"));
            }
        }

        fileMenu.addSeparator();
        fileMenu.add(GuiUtils.setIcon(GuiUtils.makeMenuItem("Export to Plugin", this, "exportToPlugin"),
                                      "/auxdata/ui/icons/plugin_add.png"));
    ...
    public void editInExternalEditorInner(final LibHolder holder) {
        try {
            ...
            holder.jythonEditor.setEnabled(false);
            ...
            holder.jythonEditor.setEnabled(true);
            holder.wrapper.add(BorderLayout.CENTER, holder.jythonEditor);
            holder.wrapper.repaint();
    ...
    private void applyJythonResources(PythonInterpreter interp, ResourceCollection resources) {
        ...
        for (int i = resources.size() - 1; i >= 0; i--) {
            ...
            try {
                ...
                LogUtil.userErrorMessage("Syntax error in the Jython library:" + resourceName + "\n" + pse);
                inError = true;
            } catch (Exception exc) {
    ...
    private boolean evaluateLibJython(boolean forWriting, LibHolder holderToWrite) {
        ...
        try {
            ...
            try {
                if (forWriting) {
                    if (GuiUtils.showYesNoDialog(null, "There was an error in the Jython library:" + pse,
                                                 "Jython error", "Save anyways", "Cancel")) {
                        return true;
                    }
                } else {
    ...
    public void dataGroupsChanged() {
        for (int dddIdx = 0; dddIdx < descriptors.size(); dddIdx++) {
            DerivedDataDescriptor ddd = (DerivedDataDescriptor) descriptors.get(dddIdx);
    ...
    protected void initUserFormulas(IdvResourceManager newIrm) {
        ...
        try {
            descriptors = DerivedDataDescriptor.init(getIdv(), newIrm.getXmlResources(newIrm.RSC_DERIVED));
    ...
    protected List selectFormulas() {
        ...
        JPanel      contents    =
            GuiUtils.topCenter(GuiUtils.inset(GuiUtils.cLabel("Please select the formulas you would like to export"),
                4), scroller);
    ...
    protected void writeUserFormulas() {
        ...
        try {
            String           filename = getResourceManager().getXmlResources(IdvResourceManager.RSC_DERIVED).getWritable();
    ...
    public void doMakeDataChoiceMenuItems(DataChoice dataChoice, List items) {
        ...
        items.add(mi);
        items.add(GuiUtils.makeMenuItem("Export to Plugin", getIdv().getPluginManager(), "addObject", ddd));
    ...
    public List doMakeFormulaDataSourceMenuItems(DataSource dataSource) {
        ...
        menuItems.add(GuiUtils.setIcon(GuiUtils.makeMenuItem("Create Formula", this, "showFormulaDialog"),
                                       "/auxdata/ui/icons/formula_add.png"));
        menuItems.add(GuiUtils.setIcon(GuiUtils.makeMenuItem("Edit Jython Library", this, "showJythonEditor"),
                                       "/auxdata/ui/icons/EditJython16.gif"));
        menuItems.add(GuiUtils.setIcon(GuiUtils.makeMenuItem("Import Formulas", this, "importFormulas"),
                                       "/auxdata/ui/icons/formula_import.png"));
        menuItems.add(GuiUtils.setIcon(GuiUtils.makeMenuItem("Export Formulas", this, "exportFormulas"),
                                       "/auxdata/ui/icons/formula_export.png"));
        menuItems.add(GuiUtils.setIcon(GuiUtils.makeMenuItem("Export Formulas to Plugin", this,
                "exportFormulasToPlugin"), "/auxdata/ui/icons/plugin_add.png"));

        if (dataSource instanceof DescriptorDataSource) {
            menuItems.add(GuiUtils.makeMenu("Edit Formulas", doMakeEditMenuItems((DescriptorDataSource) dataSource)));
        }
    ...
    public List doMakeEditMenuItems(DescriptorDataSource dds) {
        ...
        for (int i = 0; i < descriptors.size(); i++) {
            ...
            JMenu                 catMenu  = null;
            String                catSoFar = "";
            JMenuItem             mi       = GuiUtils.makeMenuItem(GuiUtils.getLocalName(ddd.getDescription(), ddd.getIsLocalUsers()),
                               this, "showFormulaDialog", ddd);
            ...
            if ((methodName != null) && (seenMethods.get(methodName) == null)) {
                ...
                if ((i1 >= 0) && (i2 >= 0)) {
                    ...
                    if ((i1 != i2) && (seenPaths.get(fullPath) == null)) {
                        ...
                        derivedDataInterpreter.exec("from " + pkg + " import " + className);
                    }
                }
            }
        }
    ...
    public List makeProcedureMenu(final Object object, final String method, final String prefix) {
        ...
        for (int i = 0; i < holders.size(); i++) {
            ...
            final JythonManager.LibHolder libHolder = (JythonManager.LibHolder) holders.get(i);
    ...
    public List findJythonMethods(boolean justList, List holders) {
        ...
        for (int i = 0; i < holders.size(); i++) {
            List                    subItems  = new ArrayList();
            JythonManager.LibHolder libHolder = (JythonManager.LibHolder) holders.get(i);
            List                    funcs     = libHolder.getFunctions();
    ...
    public static class LibHolder extends TextSearcher.TextWrapper {
         ...
         */
        public LibHolder(boolean editable, JythonManager jythonManager, String label, MyJythonEditor editor,
                         String filePath, JComponent wrapper) {
            this.jythonEditor = editor;
            setTextComponent(jythonEditor.getTextComponent());
            ...
            if (editable) {
                saveBtn = GuiUtils.makeButton("Save", jythonManager, "writeJythonLib", this);
                bottom  = saveBtn;
            }

            this.outerContents = GuiUtils.topCenterBottom(GuiUtils.inset(new JLabel(label), 5), wrapper,
                    GuiUtils.wrap(saveBtn));

            if (!editable) {
                jythonEditor.getTextComponent().setEditable(false);
                jythonEditor.getTextComponent().setBackground(COLOR_DISABLED);
                jythonEditor.getLineNumberComponent().setBackground(COLOR_DISABLED);
            }
        }
        ...
        public List getFunctions() {
            if (functions == null) {
                ...
                for (int itemIdx = 0; itemIdx < items.__len__(); itemIdx++) {
                    ...
                    functions.add(new Object[] { pair.__finditem__(0).toString(), pair.__finditem__(1) });
                }
    ...
    /**
     * Class MyJythonEditor the editor class
     *
     *
     * @author IDV Development Team
     * @version $Revision: 1.3 $
     */
    private static class MyJythonEditor extends JPythonEditor {
         ...
         * @throws VisADException on badness
         */
        public MyJythonEditor() throws VisADException {}
