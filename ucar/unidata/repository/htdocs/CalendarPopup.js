...
function PopupWindow_hidePopupWindows(e){if(window.popupWindowOldEventListener!=null){window.popupWindowOldEventListener(e);}
for(var i=0;i<popupWindowObjects.length;i++){if(popupWindowObjects[i]!=null){var p=popupWindowObjects[i];p.hideIfNotClicked(e);}}}
function PopupWindow_attachListener(){if(document.layers){document.captureEvents(Event.MOUSEUP);}
window.popupWindowOldEventListener=document.onmouseup;if(false&&window.popupWindowOldEventListener!=null){document.onmouseup=new Function("window.popupWindowOldEventListener(); PopupWindow_hidePopupWindows();");}else{document.onmouseup=PopupWindow_hidePopupWindows;}}
...
var date_class;if(this.type!="WINDOW"){result+="<TABLE WIDTH=160 BORDER=0 BORDERWIDTH=0 CELLSPACING=0 CELLPADDING=0>";}
