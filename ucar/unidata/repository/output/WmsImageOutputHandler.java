...
public class WmsImageOutputHandler extends OutputHandler {
    ...
    public static final OutputType OUTPUT_WMS_CAPABILITIES =
        new OutputType("WMS Capabilities", "wms.capabilities", OutputType.TYPE_NONHTML, "",
                       ICON_IMAGE);
    ...
    public void getEntryLinks(Request request, State state, List<Link> links)
        ...
        links.add(makeLink(request, state.getEntry(), OUTPUT_WMS_CAPABILITIES));
    ...
    public Result outputCapabilities(Request request, 
        ...
        throws Exception {
        if(wmsTemplate==null) {
            wmsTemplate = getRepository().getResource("/ucar/unidata/repository/output/wmstemplate.xml");
        }
        String url = getRepository().absoluteUrl(request.entryUrl(getRepository().URL_ENTRY_SHOW, entry, ARG_OUTPUT, OUTPUT_WMS_CAPABILITIES));
        ...
        wms  = wms.replace("${abstract}",XmlUtil.encodeString(entry.getDescription()));

        StringBuffer layers = new StringBuffer();
        for(Entry layerEntry: entries) {
            if(!isLatLonImage(layerEntry)) continue;
            String layer  = layerTemplate;
            layer = layer.replace("${name}", layerEntry.getId());
            layer = layer.replace("${title}", XmlUtil.encodeString(layerEntry.getName()));
