 ...
 */
public class GridCoverageFlatField extends CachedFlatField {
    ...
    /** the geogrid */
    transient private Coverage geoGrid;
     ...
     */
    public GridCoverageFlatField(Coverage geoGrid, SubsetParams params, Object readLock, int timeIndex,
     ...
     * @throws VisADException On badness
     */
    public GridCoverageFlatField(GridCoverageFlatField that, boolean copy,
    ...
    public CachedFlatField cloneMe(boolean copy, FunctionType type,
        ...
        CachedFlatField ccf = new GridCoverageFlatField(this, copy, type,
     ...
     * @throws VisADException On badness
     */
    public GridCoverageFlatField(FunctionType type, Set domainSet,
            ...
            synchronized (getReadLock()) {
                ...
                try {
                   ...
                   //         -1);
                    ...
                    double [] levels = geoGrid.getCoordSys().getZAxis().getValues();
                    double [] vIntv = this.vIntv;
                    double [] ilevels = new double[levels.length];
                    if(vIntv == null)
                        numLevels = 1;
                    else {
                        for (int i = 0; i < levels.length; i++) {
                            if (levels[i] >= vIntv[0] && levels[i] <= vIntv[1]) {
                    ...
                    if(vIntv == null){
                        ...
                        for (int j = 0; j < numLevels; j++) {
                            double[] zIntv = {ilevels[j], ilevels[j]};
                            subsetParams.setVertCoordIntv(zIntv);
                            GeoReferencedArray mySubset = geoGrid.readData(subsetParams);
