...
public class GridVertCoord implements Comparable {
  ...
  GridVertCoord(List records, String levelName, GridTableLookup lookup) {
    ...
    if (debug)
      System.out.println("GribVertCoord: "+getVariableName()+"("+typicalRecord.getLevelType1()+") useVertical= "+
    ...
    for (int i = 0; i < records.size(); i++) {
      ...
      if (coordIndex(record) < 0) {
        ...
        if (dontUseVertical && levels.size() > 1) {
          if (debug)
            logger.warn("GribCoordSys: unused level coordinate has > 1 levels = "+levelName+" "+record.getLevelType1()+" "+levels.size());
        }
      }
    }
    Collections.sort( levels );
