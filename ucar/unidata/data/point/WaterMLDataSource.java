...
public class WaterMLDataSource extends PointDataSource {
	...
	List<Element> timeSeriesNodes = (List<Element>)XmlUtil.findChildren(root, WaterMLUtil.TAG_TIMESERIES);
    ...
    <sourceInfo xsi:type="SiteInfoType">
      ...
      <geoLocation>
	    ...
	    */
	    ...
	    String name = XmlUtil.getGrandChildText(variableNode, WaterMLUtil.TAG_VARIABLENAME);
	    String unitName = XmlUtil.getGrandChildText(variableNode, WaterMLUtil.TAG_UNITS);
	    ...
	    Element valuesNode = XmlUtil.findChild(timeSeriesNode,WaterMLUtil.TAG_VALUES);
	    List<Element> valueNodes = (List<Element>)XmlUtil.findChildren(valuesNode, WaterMLUtil.TAG_VALUE);
	    int cnt = 0;
	    for(Element valueNode: valueNodes) {
		cnt++;
		///xxxx
		//            <value dateTime="2001-01-01T00:00:00">11.2673</value>
		Date date = sdf.parse(XmlUtil.getAttribute(valueNode, WaterMLUtil.ATTR_DATETIME));
		EarthLocation elt =
		...
		double   value   = new Double(XmlUtil.getChildText(valueNode).trim()).doubleValue();
