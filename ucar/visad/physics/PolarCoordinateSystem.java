/*
 * $Id: PolarCoordinateSystem.java,v 1.8 2006/03/17 17:08:52 jeffmc Exp $
 *
...
public class PolarCoordinateSystem extends visad.CoordinateSystem {
     ...
     */
    protected PolarCoordinateSystem(RealTupleType ref, Unit magUnit, Unit angUnit)
        ...
        if ( !Unit.canConvert(refXUnit, refYUnit)) {
            throw new CoordinateSystemException(
                getClass().getName() + ".<init>(): Reference X-unit ("
                + refXUnit + ") not convertible with reference Y-unit ("
                + refYUnit + ")");
        }

        if ( !Unit.canConvert(magUnit, refXUnit)) {
            throw new CoordinateSystemException(
                getClass().getName() + ".<init>(): Magnitude unit ("
                + magUnit + ") not convertible with reference X-unit ("
                + refXUnit + ")");
        }

        if ( !Unit.canConvert(angUnit, PlaneAngle.RADIAN)) {
            throw new CoordinateSystemException(
                getClass().getName() + ".<init>(): " + "Plane-angle unit ("
                + angUnit + ") not convertible with radians");
        }
     ...
     */
    public static PolarCoordinateSystem instance(RealTupleType ref, Unit magUnit, Unit angUnit)
            throws UnitException, VisADException {
