...
public class LocalFileTypeHandler extends GenericTypeHandler {
     ...
     */
    public LocalFileTypeHandler(Repository repository, Element entryNode) throws Exception {
    ...
    public File getFileFromId(String id, File baseFile) throws Exception {
        if(id==null || id.length()==0) return baseFile;
        String subPath =  new String(XmlUtil.decodeBase64(id));
        File file = new File(IOUtil.joinDir(baseFile, subPath ));
        if(!IOUtil.isADescendent(baseFile, file)) throw new IllegalArgumentException ("Bad file path:" +subPath);
        ...
        if(!ok) {
            if(getRepository().getLocalFilePaths().size()==0) {
                throw new IllegalArgumentException("For security you must specify the allowable  file paths in the Administration screen");
            }
            throw new IllegalArgumentException("The specified file is not under one of the allowable file system directories");
        }
    ...
    public List<String> get(Object[]values,int idx) {
        if(values[idx]  == null) return    new ArrayList<String>();
        return (List<String>)StringUtil.split(values[idx],"\n",true,true);
    }


    public List<String> getSynthIds(Request request, Group parentEntry, String synthId) throws Exception {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if(age !=0 && ( now - childFile.lastModified())<age) continue;
    ...
    private boolean match(File file, List<String> patterns, boolean dflt) {
        ...
        for(String pattern: patterns) {
            if(pattern.startsWith("dir:")) {
                if(file.isDirectory()) {
                    hadPattern = true;
                    pattern = getRegexp(pattern.substring("dir:".length()));
                    if(StringUtil.stringMatch(value, pattern,true,false)) return true;
                }
            } else {
                if(!file.isDirectory()) {
                    hadPattern = true;
                    if(StringUtil.stringMatch(value, getRegexp(pattern),true,false)) {
                        return true;
                    }
                }
            }
        }
        if(hadPattern) return false;
        return dflt;
    }
